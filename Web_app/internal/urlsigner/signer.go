...
// Expired
func (s *Signer) IsTokenExpired(token string, minutesUntilExpiration int) bool {
	crypt := goalone.New(s.Secret, goalone.Timestamp)
	ts := crypt.Parse([]byte(token))

	var expired bool
	expired = time.Since(ts.Timestamp) > time.Duration(minutesUntilExpiration)*time.Minute
